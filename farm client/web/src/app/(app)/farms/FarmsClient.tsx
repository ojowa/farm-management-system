...
import { useFarms, useCreateFarm } from '@farm/hooks';
...
export function FarmsClient({ initialFarms }: FarmsClientProps) {
  ...
  const { data, isLoading } = useFarms(farmsAPI);
  const createFarm = useCreateFarm(farmsAPI);

  const farms = useMemo(() => {
    if (data) {
      const items = data?.data?.farms || data?.data || data?.farms || data || [];
  ...
  const filtered = useMemo(() => {
    ...
    if (farmTypeFilter) {
      result = result.filter((f: any) => f.farmType === farmTypeFilter);
    }
    return result;
  ...
  return (
    <div>
      ...
      <FilterBar
        searchValue={search}
        onSearchChange={(val) => { setSearch(val); handlePageReset(); }}
        ...
        onClear={() => { setSearch(''); setStatusFilter(''); setFarmTypeFilter(''); handlePageReset(); }}
      />
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); clearErrors(); }} title="Add Farm">
        <form onSubmit={handleAdd}>
          ...
          <div className="flex justify-end gap-3 mt-4">
            ...
            <Button type="submit" loading={createFarm.isPending}>Create Farm</Button>
          </div>
        </form>
      </Modal>
    </div>
