...
export const login = createAsyncThunk(
  ...
  async ({ email, password }: { email: string; password: string }, { rejectWithValue }) => {
    try {
      ...
      if (data.mfaRequired) {
        return { requiresMFA: true, mfaToken: data.mfaSessionToken, user: data.user };
      }
      ...
      return {
        ...
        socketAccessToken: data.accessToken || null,
      };
    } catch (err: any) {
...
export const refreshSocketToken = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await apiClient.client.post('/auth/refresh', {});
