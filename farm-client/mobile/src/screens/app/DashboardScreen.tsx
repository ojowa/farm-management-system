...
export default function DashboardScreen() {
  ...
  const loadDashboardData = useCallback(async (isSilent = false) => {
    try {
      ...
      // Check if all primary data streams failed
      const allCoreFailed =
      ...
      // Safe extraction from fulfilled responses
      const farms = farmsRes.status === 'fulfilled' ? extractArray<RawFarm>(farmsRes.value).map(transformFarm) : [];
      const crops = cropsRes.status === 'fulfilled' ? extractArray<any>(cropsRes.value) : [];
      const livestock = livestockRes.status === 'fulfilled' ? extractArray<RawLivestock>(livestockRes.value).map((item) => transformLivestock(item)) : [];
      const poultry = poultryRes.status === 'fulfilled' ? extractArray<RawFlock>(poultryRes.value).map(transformFlock) : [];

      let totalRevenue = 0;
      if (financeRes.status === 'fulfilled') {
        const financeRaw = extractArray<any>(financeRes.value);
  ...
  // Refresh when returning to this tab screen
  useFocusEffect(
    useCallback(() => {
