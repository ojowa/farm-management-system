...
export class PoultryRepository {
  ...
  // --- Medication CRUD ---
  async createMedication(data: { flockId: string; name: string; dosage: string; frequency: string; startDate: Date; endDate?: Date | null; notes?: string | null }) {
  ...
  async getAllMedications(filter: any = {}, sortBy: string = 'createdAt', sortOrder: 'asc' | 'desc' = 'desc', page: number = 1, limit: number = 20) {
    const skip = (page - 1) * limit;
    const where: any = {};
    if (filter.flockId) where.flockId = filter.flockId;
    if (filter.status) where.status = filter.status;
    if (filter.search) {
      where.OR = [
        ...
        { dosage: { contains: filter.search, mode: 'insensitive' as const } },
      ];
    }

    const [data, total] = await Promise.all([
      prisma.medication.findMany({ where, include: { flock: true }, orderBy: { [sortBy]: sortOrder }, skip, take: limit }),
      prisma.medication.count({ where }),
    ]);
    return { data, total, page, totalPages: Math.ceil(total / limit) };
  }

  async updateMedication(id: string, data: { flockId?: string; name?: string; dosage?: string; frequency?: string; startDate?: Date; endDate?: Date | null; notes?: string | null; status?: string }) {
