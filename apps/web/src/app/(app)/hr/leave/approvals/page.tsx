...
export default function LeaveApprovalsPage() {
  ...
  const handleReject = async () => {
    ...
    try {
      await apiClient.put(`/leave/requests/${rejectModal}/reject`, { rejectionReason: rejectReason || undefined });
            ...
            <Card key={req.id}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400 mt-1">Requested: {new Date(req.createdAt).toLocaleDateString()}</p>
                </div>
                {req.status === 'PENDING' && (
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Reject Leave Request</h2>
            <div>
