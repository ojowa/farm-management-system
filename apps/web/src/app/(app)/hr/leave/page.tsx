...
export default function LeavePage() {
  ...
  async function loadLeaveTypes() {
    try {
      ...
      setLeaveTypes(data.filter((t: any) => t.isActive));
    } catch { /* ignore */ }
  }

  const handleRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.leaveTypeId || !form.startDate || !form.endDate) { toastError('All fields required'); return; }
    setSaving(true);
    try {
      ...
      toastError(err.response?.data?.error || 'Failed to submit');
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          ...
          {filtered.length === 0 ? (
            <Card><div className="text-center py-12 text-gray-500">No leave requests found</div></Card>
          ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium text-gray-900 dark:text-white">{req.leaveType?.name || 'Unknown'}</span>
                      ...
                      {req.rejectionReason && <p className="text-sm text-red-500 mt-1">Reason: {req.rejectionReason}</p>}
                    </div>
                    {req.status === 'PENDING' && (
                      <button onClick={() => handleCancel(req.id)} className="text-xs text-red-600 hover:text-red-800">Cancel</button>
                    )}
                  </div>
                </Card>
              ))}
        ...
        <div className="space-y-3">
          {balance.length === 0 ? (
            <Card><div className="text-center py-12 text-gray-500">No leave balance configured</div></Card>
          ) : (
            balance.map((b) => (
              <Card key={b.leaveTypeId}>
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-medium text-gray-900 dark:text-white">{b.leaveTypeName}</span>
                    {b.isPaid ? <Badge color="green">Paid</Badge> : <Badge color="gray">Unpaid</Badge>}
                  </div>
                  <div className="flex gap-4 text-sm">
                    <div className="text-center">
                      <p className="text-gray-400 text-xs">Total</p>
                      <p className="font-semibold text-gray-900 dark:text-white">{b.totalDays}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-gray-400 text-xs">Used</p>
                      <p className="font-semibold text-orange-600">{b.usedDays}</p>
                    </div>
                    <div className="text-center">
